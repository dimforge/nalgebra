...
Resizing:
  ...
  * `.resize(nrows, ncols, ...)` resizes `self` such that it contains `nrows` rows and `ncols` columns.
  * `.fixed_resize<R, C>(...)` resizes `self` such that it contains `R` rows and `C` columns.
